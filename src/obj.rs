...
pub fn load_obj<P: AsRef<Path> + ?Sized>(p: &P) -> Result<(Vec<Pnt3>, Vec<Vec3>)> {
    fn parse_f(tok: &str) -> Result<(isize, Option<isize>)> {
        let s = tok.split('/').collect::<Vec<_>>();
        Ok(match s.len() {
            1 => (tok.parse::<isize>().unwrap() - 1, None),
            3 => (s[0].parse::<isize>().unwrap() - 1, Some(s[2].parse::<isize>().unwrap() - 1)),
    ...
    for line in BufReader::new(File::open(p).unwrap()).lines() {
        ...
        if toks[0] == "v" {
            ...
            if a.1.is_none() {
                ...
                let na = obj_norms[n_i(a.1.unwrap())];
                let nb = obj_norms[n_i(b.1.unwrap())];
                let nc = obj_norms[n_i(c.1.unwrap())];
                n = (na, nb, nc);
            }
